...
from agent.prompts import SUMMARIZE_WEB_SEARCH
...
def summarize_webpage_content(webpage_content: str) -> Summary:
    ...
    try:
        ...
        return Summary(
            ...
            "..." if len(webpage_content) > 1000 else webpage_content
        )
    ...
    for result in results.get('results', []):
        ...
        try:
            ...
            summary_obj = Summary(
                ...
                summary=result.get(
                    'content', f'Could not fetch URL (timeout/connection error). Try another search.')
            )

        # uniquify file names
        uid = base64.urlsafe_b64encode(
            uuid.uuid4().bytes).rstrip(b"=").decode("ascii")[:8]
